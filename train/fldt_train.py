...
class FldtMatch():
    ...
    def cal_class_weight(self, selected_label, classwise_weight):
        # 从全局累计有效标签中得到每个类的学习难度
        effective_label = torch.sum(selected_label != -1) + self.all_label_number
        pseudo_counter = Counter(selected_label.tolist())   # 记录元素模型 peseudo_counter == sigma_t(c)
        for _, k in enumerate(pseudo_counter.keys()):
            if k != -1:
                classwise_weight[k] = pseudo_counter[k]+self.per_label_number[k]
        classwise_weight = classwise_weight / max(classwise_weight)
        return pseudo_counter, classwise_weight
    ...
    def train(self):
        ...
        classwise_weight = torch.zeros((self.num_classes)).to(device)
        ...
        for e in range(0, self.epoch):
            ...
            for input_l, input_u in zip(self.train_loader, self.un_loader):
                ...
                pseudo_counter, classwise_weight = self.cal_class_weight(selected_label, classwise_weight)
                ...
                # mask = max_probs.ge(threshold * (classwise_acc[targets_u] / (2. - classwise_acc[targets_u]))).float()  # convex 使用这种的话，不平衡无标签数据中少类别置信度一直都低
                # mask = max_probs.ge(threshold * (3. + classwise_acc[targets_u] / (5. - classwise_acc[targets_u]))).float()
                # a = torch.log10(classwise_acc[targets_u]+9)
                a = torch.log10(torch.tensor(n-1+classwise_weight[targets_u]))/torch.log10(torch.tensor(n))
                mask = max_probs.ge(self.threshold * a).float()
            ...
            info = "E|L:{}|{} lr:{} num_use:{} weight:{} {}|{} ".format(
                ...
                list(classwise_weight.cpu().detach().numpy()),
